...
public class FileHandler {
    ...
    public void save_model(Model model, String name){
        ...
        int states = model.get_transition_weights().length;
        int inputs = model.get_transition_weights()[0][0].length;
        BitSet mask = model.get_transition_bit_mask();
    ...
    private static ArrayList<String> get_strings(Model model) {
        ArrayList<String> content = new ArrayList<>();

        for (int i = 0; i< model.get_emission_weights().length; i++){
            for (int j = 0; j< model.get_emission_weights()[i].length; j++){
                for (int k = 0; k < model.get_emission_weights()[i][j].length; k++){
                    content.add(i + "\t" + j + "\t" + k + "\t" + model.get_emission_weights()[i][j][k]);
                }
            }
        }

        for (int i = 0; i< model.get_transition_weights().length; i++){
            for (int j = 0; j< model.get_transition_weights()[i].length; j++){
                for (int k = 0; k < model.get_transition_weights()[i][j].length; k++){
                    content.add(i + "\t" + j + "\t" + k + "\t" + model.get_transition_weights()[i][j][k]);
                }
            }
        }
        return content;
    ...
    public ArrayList<Model> load_models() {
    ...
    public Model load_model(String file_name){
        ...
        int index = 4;
        Model model = new HMM(game, states, inputs);
        model.set_name(name);
        model.set_transition_bit_mask(mask);

        model.set_emission_weights(new double[states][4][inputs]);
        for (int i=0; i<states * 4 * inputs; i++){
            ...
            model.get_emission_weights()[a][b][c] = w;
            index ++;
        }

        model.set_transition_weights(new double[states][states][inputs]);
        for (int i=0; i<states * states * inputs; i++){
            ...
            int c = Integer.parseInt(elements[2]);
            double w = Double.parseDouble(elements[3]);
            model.get_transition_weights()[a][b][c] = w;
            index ++;
        }
